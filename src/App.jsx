...
export default function App() {
  ...
  const [loadedDates, setLoadedDates] = useState({}); // which dates have been read from storage
  const [editingPast, setEditingPast] = useState(false); // is the current past day in edit mode
  ...
  const showEditable = isToday || editingPast; // today is always editable; past days only after tapping Edit
  ...
  // Load a day's saved data from storage the first time we navigate to it.
  // Without this, past days render blank in memory and the save effect below
  // would overwrite stored data with an empty day. (This was the data-loss bug.)
  useEffect(() => {
    if (!initialized) return;
    if (loadedDates[activeDate]) return; // already loaded
    let cancelled = false;
    (async () => {
  ...
  // Persist a day ONLY after it has been loaded, so navigation never clobbers
  // stored data with a blank in-memory day.
  useEffect(() => {
    if (!initialized) return;
    if (!loadedDates[activeDate]) return; // do not save a day we haven't loaded
  ...
  // Reset the past-day edit toggle whenever the date changes.
      ...
      <div style={{ maxWidth:600, margin:"0 auto", padding:"13px 12px 0" }}>
        ...
        {!isToday && (
          <div style={{ display:"flex", gap:8, marginBottom:12 }}>
            <button onClick={() => setActiveDate(todayKey())} style={{ flex:1, padding:"6px", borderRadius:8, fontSize:11, background:"transparent", border:`1px solid ${C.border}`, color:C.accentDim, cursor:"pointer" }}>← Back to today</button>
            {!editingPast
            ...
            <div key={meal.id} style={{ background:C.card, border:`1px solid ${C.border}`, borderRadius:14, marginBottom:12, overflow:"hidden" }}>
              ...
              <div style={{ padding:"6px" }}>
                {showEditable ? (
                  ...
                  // Past-day SUMMARY: only foods that were eaten (checked), read-only.
                  (() => {
                    ...
                    return eaten.map(row => {
                      const food = foodMap[row.foodId]; if (!food) return null;
                      const n = scaleNutrients(food, row.qty, ["protein","kcal"]);
                      return (
